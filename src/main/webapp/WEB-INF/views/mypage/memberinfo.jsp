...
<script>
	...
	function orderRefund(no){
		$("#orderNo").val(no);
	}
	
	function refund2() {
		let url = "/userRefundBoard/insertRefund"
		let code = $("select[name=refund] option:selected").val();
		console.log(userId);
		if(code == ""){
			alert("환불 사유를 선택하세요");
			return;
	...
	function parsingOrder(data) {
		...
		$
				.each(
						...
						function(i, e) {
							...
							output2 += '<td><div class="media"><div class="d-flex" style="width:200px"><img src="'+e.cover+'" style="width: 150px"></div><div class="media-body"><p>'
									+ e.title + '</p></div></div></td>'
							output2 += '<td>' + e.productQtt + '</td>'
							output2 += '<td><h5 id = "price'+e.cartNo+'">'
									+ e.price.toLocaleString() + '원</td>'
							output2 += '<td>';
							...
							if (e.refundNo != 0) {
								...
								output2 += '<button type="button" id="refundBtn" data-bs-toggle="modal" data-bs-target="#myModal" onclick="orderRefund(this.value);" class="button" value="'
										+ e.orderNo + '">환불신청</button>';
							}
							output2 += '</td>';
							output2 += '<td>';
							if (e.refundNo != 0 
								...
								output2 += '<button type="button" id="cofirmBtn" class="button" onclick="confirm(this.value);" value="'
										+ e.orderNo + '">확정하기</button>';
							}
							output2 += '</td></tr>';
						});
		if (pageNo != 1) {
			...
			pagingoutput += "<li class='page-item last'><a class='page-link'onclick='getOrderStatus("
					+ (pageNo - 1)
		...
		for (let i = data.pagingInfo.startNoOfCurPagingBlock; i <= data.pagingInfo.endNoOfCurPagingBlock; i++) {
			if (pageNo == i) {
				pagingoutput += "<li class='page-item active'><a class='page-link' onclick='getOrderStatus("
						+ i + ")'>" + i + "</a></li>";
			} else {
				pagingoutput += "<li class='page-item'><a class='page-link' onclick='getOrderStatus("
						+ i + ")'>" + i + "</a></li>";
			}
		}
					...
					.ajax({
						...
						success : function(data) { // 통신 성공시 실행될 콜백 함수
							if (data == 1) {
								...
								$(".modal-footer")
										.prepend(
												"<p style = 'text-align : left;'>비밀번호 변경이 완료되었습니다.</p>");
				...
				.ajax({
					...
					success : function(data) { // 통신 성공시 실행될 콜백 함수
						...
						$.each(data, function(i, e) {
							...
							output += '<tr><td>' + e.point + '</td><td>'
									+ e.pointWhy + '</td><td>' + e.pointWhen
									+ '</td></tr>'
						})
		...
		$
				.ajax({
					...
					success : function(data) { // 통신 성공시 실행될 콜백 함수
						...
						$.each(data, function(i, e) {
							//let date = new Date(e.pointWhen);
							output += '<tr><td>' + e.pointWhen + '</td><td>'
									+ e.point + '</td><td>' + e.pointWhy
									+ '</td></tr>'
							sum += e.point
						})
						output += '<tr><td>총 적립 포인트 : </td><td>' + sum
					...
					success : function(data) { // 통신 성공시 실행될 콜백 함수
						...
						$
								.each(
										...
										function(i, e) {
											//let date = new Date(e.pointWhen);
											output += '<tr><td><input type = "checkbox" name = "PostNo" value="'+ e.question_no + '"></td><td>'
													+ e.question_no
													+ '</td><td>'
													+ e.content
													+ '</td><td>'
					...
					success : function(data) { // 통신 성공시 실행될 콜백 함수
						...
						$
								.each(
										...
										function(i, e) {
											//let date = new Date(e.pointWhen);
											output += '<tr><td><input type = "checkbox" name = "PostNo" value="'+ e.boardno + '"></td><td>'
													+ e.boardno
													+ '</td><td>'
													+ e.title
													+ '</td><td>'
					...
					success : function(data) { // 통신 성공시 실행될 콜백 함수
						...
						$
								.each(
										...
										function(i, e) {
											//let date = new Date(e.pointWhen);
											output += '<tr><td><input type = "checkbox" name = "PostNo" value="'+ e.postNo + '"></td><td>'
													+ e.postNo
													+ '</td><td>'
													+ e.categoryCode
													+ '</td><td>'
						...
						$
								.each(
										...
										function(i, e) {
											...
											output += '<tr><td><input type = "checkbox" name = "PostNo" value="'+ e.reviewNo + '"></td><td><div class = "star"  style="display: inline-block; float: left; margin-right: 25px;">'
													+ starMark
													+ '</div></td><td>'
													+ e.title
													+ '</td><td>'
	...
	window.onload = function() {
		...
		document.getElementById('showMyPoint').addEventListener("click",
				pointHistory);
		document.getElementById('originalPwd').addEventListener("keyup",
				confirmPwd);
		document.getElementById('modifyInfo').addEventListener("click",
				modifyInfo);
		document.getElementById('showAllMyPosts').addEventListener("click",
				showMyPosts);
		document.getElementById('showMyLike').addEventListener("click",
				showMyLike);
		//showMyLike
		...
		$('#pwdModi i').on(
				...
				function() {
					...
					if ($('input').hasClass('active')) {
						$(this).attr('class', "fa fa-eye-slash fa-lg").prev(
								'input').attr('type', "text");
					} else {
						$(this).attr('class', "fa fa-eye fa-lg").prev('input')
								.attr('type', 'password');
					}
				});
							...
							<c:forEach items="${ recentSeenProd}" var="prod">
								<div class="media post_item">
									...
									<div class="media-body">
										<a
											href="http://localhost:8001/${contextPath }/product/detail?no=${prod.lastSeenProd}">
											<h3>${prod.title}</h3>
										</a>
					...
					<div class="infoBox">
						...
						<div class="single-post row">
							아이디 : <input type="text" class="form-control"
								value="${loginMember.userId}" /> 비밀번호 : <input type="hidden"
								id="password" value="${loginMember.userPwd }" />
							<button data-bs-toggle="modal" data-bs-target="#modalPwd">비밀번호
								변경</button>
							이름 : <input type="text" class="form-control"
								value="${loginMember.nickName }" /> 닉네임 : <input type="text"
								class="form-control" value="${loginMember.nickName }" /> 핸드폰 번호
							: <input type="text" class="form-control"
								value="${loginMember.phoneNum}" /> 이메일 : <input type="text"
								class="form-control" value="${loginMember.userEmail}" /> 생일 : <input
								type="date" id="userBirth" value="${loginMember.birth }" />
							<button type="button" class="button button-postComment">수정</button>
						</div>

					</div>
					<div class="mypostsBox"></div>
						...
						<div class="modal-content">
							...
							<div class="modal-body" id="pwdModi">
								현재 비밀번호 : <input type="password" class="form-control"
									id="originalPwd" placeholder="현재 비밀번호를 입력해주세요." /> <i
									class="fa fa-eye fa-lg"></i> <span id="correctOrNot"></span> <br />
								변경할 비밀번호 : <input type="text" class="form-control" id="pwd1"
									placeholder="변경할 비밀번호를 입력해주세요." /> 변경할 비밀번호 확인 : <input
									type="text" class="form-control" id="pwd2"
									placeholder="변경할 비밀번호를 다시 확인해주세요." />

							</div>
							<div class="modal-footer">
								<button type="button" class="btn btn-outline-secondary"
									id="yesButton" onclick="confirmModifyPwd(); ">확인</button>
								<button type="button" class="btn btn-primary buttonCon"
		...
		<div class="modal fade" id="myModal">
			<div class="modal-dialog">
				<div class="modal-content">
					...
					<!-- Modal Header -->
					<div class="modal-header">
						<h4 class="modal-title">환불신청</h4>
						<button type="button" class="btn-close" data-bs-dismiss="modal"></button>
					</div>

					<!-- Modal body -->
					<div class="modal-body">환불 사유</div>
					<div>
					<form action="/userRefundBoard/insertRefund" method="post">
